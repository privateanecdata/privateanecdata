...
w("   published only as part of its class's report count — no table is computed for a class, because")
w("   a class table beside its compounds' tables would publish the small ones by subtraction.")
...
w("## Mechanism classes")
w("")
w("A compound below the release threshold is published only as part of its class's report count.")
w("")
w("| Class | Members |")
w("|---|---|")
...
w("made before any data existed. The same tool runs in `real` mode on the private store before every release,")
w("as the operator's private check; its real figures are not published (exact figures would be a probe).")
w("")
