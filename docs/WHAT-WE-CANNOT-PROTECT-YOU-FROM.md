...
only grouped statistics, under rules fixed before we collected anything.
...
This is the ordinary condition of anonymous survey research, and consent forms for anonymous
studies have said so for decades. We are saying it here so it is not a surprise.
...
We make this costly (every submission requires your browser to do a small amount of work) and
detectable (we run a coordinated-submission detector and publish what it finds). We use
statistics that resist manipulation by a small number of extreme values. We do not claim the data
