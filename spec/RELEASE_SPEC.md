...
- **Unit** — anything published as one piece and computed from one set of reports: a compound's
  or class's one-way tables, one row of a table split by goal or side effect, or a pool (see
  *Cadence*).
...
published, it is updated only after at least five of those reports have changed: new ones, or
ones since excluded. Until then the earlier version is republished unchanged, marked with the
release that computed it. A table that is split into rows by goal or by side effect applies this
to each row on its own. Report counts are always current.
...
*Classes and all reports.* A class's tables are built from each of its compounds' reports as
published, plus a pool of the reports on compounds that have no tables of their own; the pool
follows the same rule, and the class's tables update whenever any of those parts does. The
all-reports tables are built the same way from the classes plus a pool of "other (not listed)"
reports. A compound's tables are first published only once at least five of its reports are
newer than the pool's last update, so that its earlier reports, already visible pooled, cannot be
separated out. Classes publish one-way tables only: outcome by goal, effect timing and stop
reasons are never pooled across a class.

*Enforcement.* One module applies all of this (`tools/update_floor.py`). `release.json` records,
for every published table, the release that computed it and the count it had then, and lists the
tables republished unchanged. `tools/verify_release.py --prior` checks that every republished
table is identical to the prior release's and, given the store, re-runs the decision on the store
as it stood at that release and requires the same answer.

This bounds what subtracting one release from the next can show to the answers of at least five
reports at once, the same protection as a table cell. Report counts move by any number; a bare
count identifies no one.
...
and for **each class**. "Per compound" below means "per compound or class," except where a
table says it is never published for a class. A compound below the
...
| Tables as of | For each compound and class whose tables are republished unchanged, the release that computed them. |
...
**Per compound × goal, never for a class:** distribution of `outcome` (no change / slight /
...
Per compound × adverse effect (only for effects at or above the cell floor in T10), never for a
class: distribution of `onset`; distribution of `dechallenge`. **Tier 4 and above.** Cross-tabulation rules apply.

### T12 — Status and discontinuation

Per compound: distribution of `status`; among stopped reports, distribution of `stop_reason` (the
stop-reason half is never published for a class).
...
source of exact goal counts: the stratum sizes T8 publishes at tier 3 are these same counts, and
T8 shows a size only for goals shown here. When a complement must be hidden here, a goal with
...
6. The release goes live. Nothing about a prior release changes. Anyone can check every step
   with `tools/verify_release.py`; see [How we count reports](../docs/RELEASE-SPEC-SUMMARY.md#how-we-count).
