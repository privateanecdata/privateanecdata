...
Generate a release from the submission store under spec/RELEASE_SPEC.md.
    ...
    python3 tools/release.py --db data/reports.db --id 2026-Q4 --date 2026-12-31 \\
        --out releases/2026-Q4 [--prior releases/2026-Q3] [--notes notes.json]
...
What it does, in order:
  ...
  5. Audits its own output: no published count below the cell floor, no row-shaped file.
...
class Tax:
    def __init__(self, path):
        ...
        self.order = [c["id"] for c in self.raw["compounds"]]           # SCHEMA.md order, never by outcome
...
def suppress(cells, groups):
    """
    Mark cells below the floor, then apply complementary suppression: in any group whose total is
    published, exactly one hidden cell would be recoverable by subtraction, so the smallest other
    cell is hidden too. Repeats until stable. Deterministic tie-break on id.
    """
    for c in cells:
    ...
    while changed:
        ...
        for g in groups:
            ...
            if len(hidden) == 1:
                ...
                if shown:
                    min(shown, key=lambda c: (c["count"], c["id"]))["suppressed"] = True
                    changed = True
...
def render_cells(cells, n, percent):
    ...
    for c in cells:
        if c["suppressed"]:
            out.append({"id": c["id"], "label": c["label"], "count": None, "display": "fewer than 5"})
        else:
...
def one_way(rows, field, options, tier, groups=(), value=None):
    ...
    suppress(cells, [[c["id"] for c in cells]] + [list(g) for g in groups])
...
def build_tables(ents, all_rows, tax, tier_total):
    """Returns {table_id: {...}} for T1–T13. Each per-entity table keyed by entity.key."""
    sh = tax.shared
    T = {k: {} for k in ["T1", "T2", "T3", "T4", "T5", "T6", "T7", "T8", "T10", "T11", "T12", "T16"]}
    outcome_opts = sh["outcome"]
    stopped_ids = [i for i, _ in sh["status"] if i.startswith("stopped-")]
    ...
    for e in ents:
        ...
        T["T4"][e.key] = {"frequency": one_way(r, "frequency", tax.opts(e.frequency), t),
                          "titration": one_way(r, "titration", sh["titration"], t)}
        T["T5"][e.key] = one_way(r, "duration", sh["duration"], t)
        T["T6"][e.key] = one_way(r, "purity_tested", sh["purityTested"], t)
        T["T7"][e.key] = one_way(r, "reconstitution", sh["reconstitution"], t)
        ...
        for row in r:
            ...
            none += not ids
            counts.update(ids)
        cells = [{"id": "none", "label": "No adverse effects reported", "count": none}]
        cells += [{"id": i, "label": l, "count": counts.get(i, 0)} for i, l in tax.opts(e.aes)]
        for c in cells:
            c["suppressed"] = c["count"] < CELL_FLOOR
        percent = t >= 4 and e.n >= PCT_DENOM_MIN
        T["T10"][e.key] = {"n": e.n, "percent": percent, "cells": render_cells(cells, e.n, percent)}
        # T12: status, then stop reason among the stopped. The stopped subtotal is the second
        # table's denominator, so it is a published group for complementary suppression.
        status = one_way(r, "status", sh["status"], t, groups=[stopped_ids])
        stopped = [row for row in r if row["status"].startswith("stopped-")]
        T["T12"][e.key] = {"status": status,
                           "stop_reason": stratum(stopped, "stop_reason", sh["stopReason"], t, "stratum", "Stopped")}
        if t < 3:
            continue
        # T8: outcome by goal. "Other" is counted as a goal but is not a stratum — there is no
        # goal to condition on.
        T["T8"][e.key] = {"n": e.n, "strata": [
            stratum([row for row in r if row["goal"] == g], "outcome", outcome_opts, t, "goal", g)
            for g in e.goals]}
        for s in T["T8"][e.key]["strata"]:
            s["label"] = tax.labels.get(s["goal"], s["goal"])
        if t < 4:
            continue
        # T11: onset and dechallenge per effect, for effects shown in T10.
        shown = [c["id"] for c in T["T10"][e.key]["cells"] if c["count"] is not None and c["id"] != "none"]
        T["T11"][e.key] = {"n": e.n, "effects": []}
        for aid in shown:
            sub = [row for row in r if any(x["id"] == aid for x in effects_of(row))]
            entry = {"effect": aid, "label": tax.labels.get(aid, aid), "n": len(sub) if len(sub) >= STRATUM_MIN else None}
            if entry["n"] is None:
                entry["display"] = "fewer than 20 reports"
            else:
                def pick(field):
                    return lambda row: next(x[field] for x in effects_of(row) if x["id"] == aid)
                entry["onset"] = one_way(sub, None, sh["onset"], t, value=pick("onset"))
                entry["dechallenge"] = one_way(sub, None, sh["dechallenge"], t, value=pick("dechallenge"))
            T["T11"][e.key]["effects"].append(entry)

    # T6 additionally overall.
    T["T6"]["overall"] = one_way(all_rows, "purity_tested", sh["purityTested"], tier_total)

    # T9: per goal, the T8 rows of every compound at tier ≥ 4 with a publishable stratum, shown
    # side by side — only when at least one participant is at tier 5 and there are at least two.
    # Everything in T9 is already public in T8; the table adds juxtaposition, never a comparison.
    T["T9"] = {}
    comps = [e for e in ents if e.kind == "compound" and e.tier >= 4]
    goals = union_in_order(e.goals for e in comps)
    for g in goals:
        parts = []
        for e in comps:
            s = next((s for s in T["T8"].get(e.key, {}).get("strata", []) if s["goal"] == g and s["n"] is not None), None)
            if s:
                parts.append({"compound": e.id, "label": e.label, "tier": e.tier, "n": s["n"], "percent": s["percent"], "cells": s["cells"]})
        if len(parts) >= 2 and any(p["tier"] >= 5 for p in parts):
            T["T9"][g] = {"goal": g, "label": tax.labels.get(g, g), "compounds": parts}

    # T13: demographics, overall only, never crossed with anything.
    if len(all_rows) >= DEMOGRAPHICS_MIN:
        declined = lambda field: (lambda r: r[field] if r[field] is not None else "declined")
        T["T13"] = {
            "n": len(all_rows),
            "age_band": one_way(all_rows, None, sh["ageBand"] + [("declined", "Preferred not to say")], 4, value=declined("age_band")),
            "sex": one_way(all_rows, None, sh["sex"] + [("declined", "Preferred not to say")], 4, value=declined("sex")),
        }
    else:
...
def build_t0(ents, tax, committed, excluded_by_reason, analyzed, other_n, prior):
    ...
    for e in ents:
        entry = {"id": e.id, "label": e.label, "tier": e.tier}
        if e.kind == "class":
            ...
            entry["count"] = e.n if e.tier >= 1 else None
            if entry["count"] is None:
...
def write_figures(T, ents, tax, out, release):
    ...
    one_way_tables = [("T1", "Route", None), ("T2", "Source channel", None), ("T16", "What people took it for", None),
                      ("T5", "Duration", None), ("T6", "Independent purity testing", None), ("T7", "Handling", None)]
    for e in ents:
        d = f"{e.kind}-{e.id}"
        ...
        if e.key in T["T4"]:
            for k, title in (("frequency", "frequency"), ("titration", "how the dose changed")):
                tbl = T["T4"][e.key][k]
                put(f"{d}/T4-{k}.svg", svg.bar_chart(f"{e.label} — {title}", sub(tbl, e), tbl, release))
        if e.key in T["T10"]:
            tbl = T["T10"][e.key]
            put(f"{d}/T10.svg", svg.bar_chart(f"{e.label} — adverse effects reported", sub(tbl, e, multi=True), tbl, release))
        if e.key in T["T12"]:
...
def sub(tbl, e=None, multi=False):
    ...
    how = "Percent of reports, with 95% interval." if tbl["percent"] else "Counts. Cells under 5 are shown as “fewer than 5”."
...
def audit(out):
    """Refuse to ship a count under the floor or a denominator under the unlock, anywhere."""
    problems = []
...
def main():
    ...
    ap.add_argument("--id", required=True, help="release id, e.g. 2026-Q4")
    ap.add_argument("--date", required=True, help="release date YYYY-MM-DD (the only timestamp in the output)")
    ap.add_argument("--out", required=True)
    ap.add_argument("--prior", help="directory of the previous release, for the received-since count")
    ...
    # 3. Entities and tiers. Compounds in taxonomy order; classes in taxonomy order.
    by_compound = {}
    for r in analyzed:
    ...
    for cid in tax.order:
        c = tax.compounds[cid]
        ents.append(Entity("compound", cid, c["label"], by_compound.get(cid, []), tax, [cid]))
    for cls in tax.classes:
        crows = [r for m in cls["members"] for r in by_compound.get(m, [])]
        ents.append(Entity("class", cls["id"], cls["label"], crows, tax, cls["members"]))
    other_n = len(by_compound.get(OTHER, []))
    tier_total = tier_of(len(analyzed))

    T = build_tables(ents, analyzed, tax, tier_total)
    ...
    T["T14"] = {"flagged_this_period": {k: sum(1 for x in period if x["reason"] == k) for k in REASON_CODES},
    ...
    T["T15"] = build_t15(analyzed, json.load(open(args.uniqueness_config)))
    ...
    titles = {"T0": "Counts", "T1": "Route", "T2": "Source channel", "T3": "Dose bands", "T4": "Frequency and titration",
              "T5": "Duration", "T6": "Purity testing", "T7": "Handling", "T8": "Outcome by goal",
    ...
    with open(os.path.join(out, "merkle", "leaves.txt"), "w") as f:
        for _, leaf in leaves:
            f.write(leaf.hex() + "\n")
    figures = write_figures(T, ents, tax, out, args.id)
    ...
    release = {
        ...
        "tiers": {e.key: e.tier for e in ents},
    ...
    print("tiers:", ", ".join(f"{e.id}={e.tier}" for e in ents if e.kind == "compound" and e.tier))
