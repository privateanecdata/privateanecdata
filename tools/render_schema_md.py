...
w("| 7 | `frequency` | That compound's frequency options | |")
w("| 8 | `titration` | 5 options | No change / stepped up / stepped down / cycled / other |")
w("| 9 | `duration` | 6 buckets | How long taken in total |")
w("| 10 | `purity_tested` | 5 options | Independent test obtained; did it match the label |")
w("| 11 | `reconstitution` | 6 options | Handling practice, coarse |")
...
w("| 14 | `status` | 6 options | Still taking, or stopped in which week bucket, or completed |")
w("| 15 | `stop_reason` | 8 options | Only if stopped |")
w("| 16 | `age_band` | 6 bands | Optional |")
w("| 17 | `sex` | 3 options | Optional |")
w("")
...
w("### Titration")
w("")
w("`no-change` · `stepped-up` · `stepped-down` · `cycled` · `other`")
w("")
w("### Purity testing")
w("")
...
w("")
w("### Reconstitution / handling")
w("")
w("`bac-water-refrigerated` · `bac-water-room-temp` · `sterile-water` · `pre-mixed-or-pen` ·")
w("`no-reconstitution` (oral, nasal, topical) · `other`")
w("")
...
w("")
w("Status: `still-taking` · `stopped-under-2wk` · `stopped-2-6wk` · `stopped-6-12wk` ·")
w("`stopped-over-12wk` · `completed-planned-course`")
w("")
w("Stop reason (if stopped): `achieved-goal` · `no-effect` · `adverse-effect` · `cost` · `supply` ·")
...
w("")
w("### Age band and sex (optional)")
w("")
w("Age: `18-24` · `25-34` · `35-44` · `45-54` · `55-64` · `65+`")
w("")
w("Sex: `male` · `female` · `prefer-not`")
w("")
w("### Routes")
w("")
w("| id | Label |")
...
w("demographics; source channel dominated by research-chemical vendors. These are pre-launch")
w("guesses. The same tool runs in `real` mode on the private store before every release, and the")
